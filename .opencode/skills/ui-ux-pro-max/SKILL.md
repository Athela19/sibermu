...
> **Node port (no Python needed):** this checkout also ships `scripts/search.mjs` (+ `core.mjs`, `design_system.mjs`, `reasoning_contract.mjs`) — a stdlib-only Node 18+ port of the Python scripts with identical flags and output. Prefer it whenever Python 3 is unavailable: replace `python3 <skill-dir>/scripts/search.py` with `node <skill-dir>/scripts/search.mjs` in every command below, e.g. `node .opencode/skills/ui-ux-pro-max/scripts/search.mjs "beauty spa wellness" --design-system -p "Serenity Spa"`.
